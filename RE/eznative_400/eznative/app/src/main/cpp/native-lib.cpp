...
jstring encrypt(JNIEnv *env, jobject obj, jstring p, int len) {
    char enc_buf[BLOWFISH_BLOCK_SIZE];
    jstring result;
    size_t *x = 0, i, j;
    char k[0x10] = "\x00\x00\x00\x00\x00\x00\x00\x00";
    BLOWFISH_KEY key;
    unsigned char *buf = (unsigned char *)malloc(len * 8);
    const char *plain = env->GetStringUTFChars(p, 0);
    LOGE("%s", env->GetStringUTFChars(p, 0));
    blowfish_key_setup((const BYTE *)k, &key, BLOWFISH_BLOCK_SIZE);
    for (i = 0; i < len; i++) {
        blowfish_encrypt((BYTE *) (plain + 8 * i), (BYTE *) enc_buf, &key);
        for (j = 0; j < BLOWFISH_BLOCK_SIZE; j++) {
            enc_buf[i] = (unsigned char)(enc_buf[i] & 0xff);
            buf[8 * i + j] = enc_buf[j];
            LOGE("%02x", buf[8 * i + j]);
        }
    }
    result = env->NewStringUTF((const char *)base64_encode((const unsigned char *)buf, len * 8, x));
    LOGE("length = %d", strlen(env->GetStringUTFChars(result, 0)));
    return result;
}
...
static const char *gClassName = "cn/b3ale/ez4u2reverse/MainActivity";
static JNINativeMethod gMethods[] = {
        {"sayHi", "()Ljava/lang/String;", (void *) sayHi},
        {"encrypt", "(Ljava/lang/String;I)Ljava/lang/String;", (void *) encrypt},
};
