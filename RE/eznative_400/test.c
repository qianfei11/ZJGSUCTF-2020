...
int main() {
    BYTE enc_buf[BLOWFISH_BLOCK_SIZE + 1];
    BLOWFISH_KEY key;
    size_t *len = 0, i, j;
    BYTE k[0x10] = "\x00\x00\x00\x00\x00\x00\x00\x00";
    blowfish_key_setup(k, &key, BLOWFISH_BLOCK_SIZE);
    BYTE p[2 * 8 + 1] = "12345678123xxxxx";
    BYTE *buf = malloc(2 * 8);
    for (i = 0; i < 2; i++) {
        blowfish_encrypt(p + 8 * i, enc_buf, &key);
        printf("Ciphertext => %s", enc_buf);
    ...
    for (i = 0; i < 16; i++)
        printf("%02x\n", buf[i]);
    BYTE *b = base64_encode(buf, 16, len);
    printf("%s\n", b);
    BYTE c[BLOWFISH_BLOCK_SIZE + 1];
    memcpy(c, enc_buf, BLOWFISH_BLOCK_SIZE);
    blowfish_decrypt(c, enc_buf, &key);
    printf("Plaintext => %s", enc_buf);
    return 0;
}
